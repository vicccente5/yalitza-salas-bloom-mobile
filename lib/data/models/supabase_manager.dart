...
class SupabaseManager {
  ...
  // Métodos de carga desde Supabase
  Future<void> loadAllData() async {
    if (!_isInitialized) return;

    try {
      print('DEBUG: Loading all data from Supabase...');
  ...
  Future<void> loadFinancialData() async {
    try {
      ...
      if (response != null) {
        _monthlyIncome = (response['monthly_income'] ?? 0.0).toDouble();
        _monthlyCosts = (response['monthly_costs'] ?? 0.0).toDouble();
        _monthlyProfit = (response['monthly_profit'] ?? 0.0).toDouble();
      }
